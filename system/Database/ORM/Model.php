...
use Npds\Support\Contracts\ArrayableInterface;
use Npds\Database\Query\Builder as QueryBuilder;
use Npds\Database\ConnectionResolverInterface as Resolver;
...
class Model implements ArrayableInterface, JsonableInterface, \ArrayAccess
{
     ...
     * @return array|static
     *
     * @throws \Npds\Database\ORM\ModelNotFoundException
     */
     ...
     *
     * @param  array|string  $relations
     * @return \Npds\Database\ORM\Builder|static
     */
     ...
     * @return \Npds\Database\ORM\Relations\HasOne
     */
     ...
     * @return \Npds\Database\ORM\Relations\BelongsTo
     */
     ...
     * @param  string  $foreignKey
     * @param  string  $localKey
     * @return \Npds\Database\ORM\Relations\HasMany
     */
     ...
     * @param  string  $otherKey
     * @param  string  $relation
     * @return \Npds\Database\ORM\Relations\BelongsToMany
     */
    ...
    public function getAttribute($key)
    {
        ...
        if (method_exists($this, $camelKey = Str::camel($key))) {
            ...
            if (! $relation instanceof Relation) {
                throw new LogicException('Relationship method must return an instance of [Npds\Database\ORM\Relation]');
            }
    ...
    {
        $event = sprintf('database.model.%s: %s', $event, get_class($callback));
    ...
    protected function performUpdate(Builder $query)
    {
        if (! empty($dirty = $this->getDirty())) {
            ...
            if (! empty($dirty = $this->getDirty())) {
                $id = Arr::array_get($this->original, $keyName = $this->getKeyName(), $this->getAttribute($keyName));
     ...
     *
     * @return \Npds\Database\ORM\Builder
     */
     ...
     *
     * @param  \Npds\Database\ORM\Model  $parent
     * @param  array   $attributes
     * @param  string  $table
     * @param  bool    $exists
     * @param  string|null  $using
     * @return \Npds\Database\ORM\Pivot
     */
     ...
     *
     * @param  \Npds\Database\ConnectionResolverInterface  $resolver
     * @return void
     */
     ...
     *
     * @param  \Npds\Events\Dispatcher  $dispatcher
     * @return void
     */
    ...
    public function toArray()
    {
        ...
        foreach ($this->getDates() as $key) {
            if (! empty($value = Arr::array_get($attributes, $key))) {
                $attributes[$key] = (string) $this->asDateTime($value);
            }
        }

        $mutatedAttributes = Arr::array_get(static::$mutatorCache, get_class($this), array());
