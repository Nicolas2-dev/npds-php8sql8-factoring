...
use Npds\Auth\RequestGuard;
use Npds\Auth\Guards\TokenGuard;
use Npds\Foundation\Application;
use Npds\Auth\Guards\SessionGuard;
...
class AuthManager
{
    ...
    public function createSessionDriver($name, array $config)
    {
        $guard = new SessionGuard(
            $name, $config['model'], $this->app['session.store'], $this->app['cookie'], $this->app['hash'], $this->app['events']
        );
     ...
     * @param  string  $name
     * @param  array  $config
     * @return \Npds\Auth\TokenGuard
     */
    public function createTokenDriver($name, $config)
    {
        $guard = new TokenGuard($config['model']);
