...
//         $message =  str_replace(array("\r\n", "\r", "\n"), "<br />", $message);
   ...
   $userX = base64_decode( (string) $user);
   $userdata = explode(':', $userX);
      ...
      if(isset($user)) {
         if ($posterdata['user_avatar'] != '') {
            if (stristr($posterdata['user_avatar'],"users_private"))
               ...
               if ($ibid=theme_image("forum/avatar/".$posterdata['user_avatar'])) {$imgava=$ibid;} else {$imgava="assets/images/forum/avatar/".$posterdata['user_avatar'];}
         }
      }
      else
         if ($ibid=theme_image("forum/avatar/blank.gif")) {$imgava=$ibid;} else {$imgava="assets/images/forum/avatar/blank.gif";}
   }
   ...
   for ($i = 0; $i < count($moderator); $i++) {
      ...
      if ($modera['user_avatar'] != '') {
         if (stristr($modera['user_avatar'],"users_private"))
            ...
            if ($ibid=theme_image("forum/avatar/".$modera['user_avatar'])) {$imgtmp=$ibid;} else {$imgtmp="assets/images/forum/avatar/".$modera['user_avatar'];}
         }
      }
   ...
   if ($allow_to_reply) {
      ...
      <div class="row">
         ...
         if ($myrow['image'] != '') {
            if ($ibid=theme_image("forum/subject/".$myrow['image'])) {$imgtmp=$ibid;} else {$imgtmp="assets/images/forum/subject/".$myrow['image'];}
            echo '<img class="n-smil" src="'.$imgtmp.'"  alt="" />';
         } 
         else {
            if ($ibid=theme_image("forum/subject/icons/posticon.gif")) {$imgtmp=$ibid;} else {$imgtmp="assets/images/forum/icons/posticon.gif";}
            echo '<img class="n-smil" src="'.$imgtmp.'" alt="" />';
         }
