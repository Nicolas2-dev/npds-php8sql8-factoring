...
include('auth.php');
   ...
   if (!$user)
      ...
      if ($dossier=='All') $Xdossier=translate("Tous les sujets"); else $Xdossier=StripSlashes($dossier);
      echo '
...
//      }
   ...
   echo '<span class="float-end">';
      if ($smilies) {
         if ($myrow['msg_image']!='') {
            if ($ibid=theme_image("forum/subject/".$myrow['msg_image'])) {$imgtmp=$ibid;} else {$imgtmp="assets/images/forum/subject/".$myrow['msg_image'];}
            echo '<img class="n-smil" src="'.$imgtmp.'" alt="icon_post" />';
            } else {
               if ($ibid=theme_image("forum/subject/00.png")) $imgtmpPI=$ibid; else $imgtmpPI="assets/images/forum/subject/00.png";
