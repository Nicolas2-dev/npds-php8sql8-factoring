...
namespace Modules\TwoCore\Library\Mailer;
...
class MailerManager
{
     ...
     * @param string $theme
     */
    public function __construct(Application $app, ThemeManager $theme, UserManager $user)
    {
        $this->app = $app;
     ...
     */
    public function send_email(string $email, string $subject, string $message, string $from = "", bool $priority = false, string $mime = "text", string $file = null): bool
    {
        $From_email = $from != '' ? $from : Config::get('two_core::config.adminmail');

        if (preg_match('#^[_\.0-9a-z-]+@[0-9a-z-\.]+\.+[a-z]{2,4}$#i', $From_email)) {
            
            $config = Config::get('two_core::mailer');
            
            if ($config['dkim_auto'] == 2) {
                //Private key filename for this selector 
                $privatekeyfile = 'storage/mailer/key/'. Config::get('app.key') .'_dkim_private.pem';

                //Public key filename for this selector 
                $publickeyfile = 'storage/mailer/key/'. Config::get('app.key') .'_dkim_public.pem';
            ...
            try {
                ...
                //Recipients 
                $mail->setFrom(Config::get('two_core::config.adminmail'), Config::get('two_core::config.sitename'));
                ...
                if ($config['dkim_auto'] == 2) {
                    ...
                    $mail->DKIM_selector = Config::get('app.key');
     ...
     * @return  void
     */
    public function copy_to_email(string|int $to_userid, string $sujet, string $message): void
    {
        $user = DB::table('users')
        ...
        if (($user['email']) and ($user['send_email'] == 1)) {
            $this->send_email($user['email'], $sujet, $message, '', true, 'html', '');
        }
    }
     ...
     * @return  void
     */    
    public function Mess_Check_Mail(string $username): void
    {
     ...
     * @return  void
     */
    public function Mess_Check_Mail_interface(string $username, string $class): void
    {
        if ($ibid = $this->theme->theme_image("fle_b.gif")) {
            $imgtmp = $ibid;
        } else {
            $imgtmp = false;
        }
     ...
     * @return  string
     */
    public function Mess_Check_Mail_Sub(string $username, string $class): string
    {
        if ($username) {
     ...
     * @return  bool
     */
    public function isbadmailuser(string|int $utilisateur): bool
    {
        $contents = '';
