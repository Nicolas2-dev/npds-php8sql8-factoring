...
if (users::autorisation(-127)) {
    $mess_adm = '<p class="text-danger">' . __d('two_geoloc', 'Rappel : vous êtes en mode administrateur !') . '</p>';
    $lkadm = '<a href="admin.php?op=Extend-Admin-SubModule&amp;ModPath=geoloc&amp;ModStart=admin/geoloc_set" title="' . __d('two_geoloc', 'Admin') . '" data-bs-toggle="tooltip"><i id="cogs" class="fa fa-cogs fa-lg"></i></a>';
    $infooo = __d('two_geoloc', 'Modification administrateur');
    // IP géoréférencées
    if ($geo_ip == 1) {
        $arpostip = array();
        ...
        while ($row_ip = sql_fetch_array($result_ip)) {
            ...
            if ($ip_lt != 0 and $ip_lg != 0) {
                ...
                switch ($ip_spam) {
                    case '':
                        ...
                        break;
                    case '5':
                        $infospam = '<span class="small badge bg-danger rounded-pill float-end tooltipbyclass" title="' . __d('two_geoloc', 'Adresse IP bannie !') . '">' . $ip_spam . '</span>';
                        break;
                    default:
                        $infospam = '<span class="small badge bg-danger rounded-pill float-end tooltipbyclass" title="' . __d('two_geoloc', 'Adresse IP signalée par l’antispam !') . '">' . $ip_spam . '</span>';
                }
                $sidebarip .= '
                <a id="ip' . $i . '" class="filtrip_js sb_js sb_ip list-group-item list-group-item-action py-1 px-2 border-left-0 border-right-0' . $cl . '"><span class="" data-bs-toggle="tooltip" title="' . __d('two_geoloc', 'Voir sur la carte') . '."><i class="me-1 bi bi-display h4 align-middle"></i></span><span class="small nlfiltrip" data-searchterm="' . urldecode($ip_ip1) . '">' . urldecode($ip_ip1) . '</span>' . $infospam . '</a>';
            ...
            <p class="col-sm-12 col-md-3 p-2  border rounded flex-column align-items-start list-group-item-action">
                ...
                <span class="ms-auto">
                <span class="badge bg-secondary ms-1" title="' . __d('two_geoloc', 'Visites') . '" data-bs-toggle="tooltip" data-bs-placement="left" >' . $ip_visite . '</span>
                </span>
                </span>
    ...
    <div id="sb_ip" class="list-group mb-2">
        <div class="bg-light text-dark fw-light p-2">
            ...
            <div class="form-check form-switch d-inline-block">
                <input class="form-check-input" type="checkbox" id="ipbox" /><label class="form-check-label" for="ipbox"><span class="text-muted" data-bs-toggle="tooltip" title="' . __d('two_geoloc', 'Voir ou masquer les IP') . '"><i class="bi bi-display me-1 h3 align-middle"></i></span>IP</label>
            </div>
        ...
        <div class="collapse" id="l_sb_ip">
            <div class="sb_ip list-group-item py-1 border-left-0 border-right-0" >
                <input id="n_filtreip" placeholder="' . __d('two_geoloc', 'Filtrer les résultats') . '" class="my-1 form-control form-control-sm" type="text" />
            </div>
            ' . $sidebarip . '
        </div>
    </div>';
...
if ($mark_typ !== 1) {
    ...
    $ic_b_mbgt = '<span title="' . __d('two_geoloc', 'Voir ou masquer membres géoréférencés') . '." data-bs-toggle="tooltip" style="font-size:1.8rem; color:' . $mbg_f_co . ';" class="fa fa align-middle">' . $fafont . '</span>';
    $ic_b_mbg = '<span title="' . __d('two_geoloc', 'Voir sur la carte') . '." data-bs-toggle="tooltip" style="font-size:1.8rem; color:' . $mbg_f_co . ';" class="fa fa align-middle">' . $fafont . '</span>';
    $ic_b_mbgct = '<span title="' . __d('two_geoloc', 'Voir ou masquer membres géoréférencés en ligne') . '." data-bs-toggle="tooltip" style="font-size:1.8rem; color:' . $mbgc_f_co . ';" class="fa fa-2x align-middle">' . $fafont . '</span>';
    $ic_b_mbgc = '<span title="' . __d('two_geoloc', 'Voir sur la carte') . '." data-bs-toggle="tooltip" style="font-size:1.8rem; color:' . $mbgc_f_co . ';" class="fa fa-2x align-middle">' . $fafont . '</span>';
    $ic_b_acgt = '<span title="' . __d('two_geoloc', 'Voir ou masquer anonymes géoréférencés') . '." data-bs-toggle="tooltip" style="font-size:1.8rem; color:' . $acg_f_co . ';" class="fa fa-2x align-middle">' . $fafont . '</span>';
    $ic_b_acg = '<span title="' . __d('two_geoloc', 'Voir sur la carte') . '." data-bs-toggle="tooltip" style="font-size:1.8rem; color:' . $acg_f_co . ';" class="fa fa-2x align-middle">' . $fafont . '</span>';
} else {
    $ic_b_mbgt = '<img src="' . $ch_img . $nm_img_mbg . '" title="' . __d('two_geoloc', 'Voir ou masquer membres géoréférencés') . '." data-bs-toggle="tooltip" alt="' . __d('two_geoloc', 'Membre géoréférencé') . '" /> ';
    $ic_b_mbg = '<img src="' . $ch_img . $nm_img_mbg . '" title="' . __d('two_geoloc', 'Voir sur la carte') . '." data-bs-toggle="tooltip" alt="' . __d('two_geoloc', 'Membre géoréférencé') . '" /> ';
    $ic_b_mbgct = '<img src="' . $ch_img . $nm_img_mbcg . '" title="' . __d('two_geoloc', 'Voir ou masquer membres géoréférencés en ligne') . '." data-bs-toggle="tooltip" alt="' . __d('two_geoloc', 'Membre géoréférencé en ligne') . '" /> ';
    $ic_b_mbgc = '<img src="' . $ch_img . $nm_img_mbcg . '" title="' . __d('two_geoloc', 'Voir sur la carte') . '." data-bs-toggle="tooltip" alt="' . __d('two_geoloc', 'Membre géoréférencé en ligne') . '" /> ';
    $ic_b_acgt = '<img src="' . $ch_img . $nm_img_acg . '" title="' . __d('two_geoloc', 'Voir ou masquer anonymes géoréférencés') . '." data-bs-toggle="tooltip" alt="' . __d('two_geoloc', 'Anonyme géoréférencé en ligne') . '" /> ';
    $ic_b_acg = '<img src="' . $ch_img . $nm_img_acg . '" title="' . __d('two_geoloc', 'Voir sur la carte') . '." data-bs-toggle="tooltip" alt="' . __d('two_geoloc', 'Anonyme géoréférencé en ligne') . '" /> ';
}
...
while ($row = sql_fetch_array($result)) {
    ...
    else $the_av_ch = 'assets/images/forum/avatar/' . $user_avatar;
}
//<== georeferencement utilisateur
...
while ($row = sql_fetch_array($result)) {
    ...
    if ($user_lat != '') { // c un membre géoréférencé
        ...
        $ip_visi_pag = '';
        //=== menu fenetre info
        $imm = ' <a href="user.php?op=userinfo&amp;uname=' . $users_uname . '"  target="_blank" ><i class="fa fa-user fa-2x me-2 tooltipbyclass" title="' . __d('two_geoloc', 'Profil') . '"></i></a>';
        if ($user)
            $imm .= ' <a href="powerpack.php?op=instant_message&to_userid=' . $users_uname . '" ><i class="far fa-envelope fa-2x me-2 tooltipbyclass" title="' . __d('two_geoloc', 'Envoyez un message interne') . '"></i></a>';
        if ($us_url != '')
            $imm .= ' <a href="' . $us_url . '" target="_blank" ><i class="fas fa-external-link-alt fa-2x me-2 tooltipbyclass" title="' . __d('two_geoloc', 'Visitez le site') . '"></i></a>';
        if ($us_mns != '')
            $imm .= '<a href="minisite.php?op=' . $users_uname . '" target="_blank" ><i class="fa fa-desktop fa-2x me-2 tooltipbyclass" title="' . __d('two_geoloc', 'Visitez le minisite') . '"></i>' . $us_mns . '</a>';
...
$sidebaronline = '
                ...
                <div class="bg-light text-dark fw-light p-2"><a id="carrets_ac" class="link" data-bs-toggle="collapse" href="#l_sb_ano"><i class="toggle-icon fa fa-caret-down fa-lg me-2" style="font-size:1.6rem;"></i></a>
                    <div class="form-check form-switch d-inline-block">
                        <input class="form-check-input" type="checkbox" checked="checked" data-bs-toggle="tooltip" title="' . __d('two_geoloc', 'Voir ou masquer anonymes géoréférencés') . '" id="conbox" /><label class="form-check-label" for="conbox"> ' . __d('two_geoloc', 'Visiteur en ligne') . '</label>
                    </div>
...
while ($row = sql_fetch_array($membre)) {
    ...
    else $av_ch = 'assets/images/forum/avatar/' . $us_user_avatar;
    //==> les membres géoréferencés
    if ($us_lat != '') {
        $mbgr++;
        //=== menu fenetre info
        $imm = '<a href="user.php?op=userinfo&amp;uname=' . $us_uname . '"  target="_blank" ><i class="fa fa-user fa-lg me-3 tooltipbyclass" title="' . __d('two_geoloc', 'Profil') . '"></i></a>';
        if ($user)
            $imm .= '<a href="powerpack.php?op=instant_message&to_userid=' . $us_uname . '" ><i class="far fa-envelope fa-lg me-3 tooltipbyclass" title="' . __d('two_geoloc', 'Envoyez un message interne') . '"></i></a>';
        if ($us_url != '')
            $imm .= '<a href="' . $us_url . '" target="_blank" ><i class="fas fa-external-link-alt fa-lg me-3 tooltipbyclass" title="' . __d('two_geoloc', 'Visitez le site') . '"></i></a>';
        if ($us_mns)
            $imm .= '<a href="minisite.php?op=' . $us_uname . '" target="_blank" ><i class="fa fa-desktop fa-lg me-3 tooltipbyclass" title="' . __d('two_geoloc', 'Visitez le minisite') . '"></i></a>';

        //==> construction du fichier json
        $cont_json .= '{"lat":' . $us_lat . ', "lng":' . $us_long . ', "html":"<img src=\\"' . $av_ch . '\\" width=\\"32\\" height=\\"32\\" align=\\"middle\\" />&nbsp;' . addslashes($us_uname) . '<br /><span class=\\"text-muted\\">' . __d('two_geoloc', 'Dernière visite') . ' : </span>' . $visit . '<br />", "label":"<span>' . addslashes($us_uname) . '</span>", "icon":"icon"},';
        $cont_geojson .= '
...
$sidebarmembres = '
                ...
                <div class="bg-light text-dark fw-light p-2"><a id="carrets_mb" class="" data-bs-toggle="collapse" href="#l_sb_member"><i class="toggle-icon fa fa-caret-down fa-lg me-2" style="font-size:1.6rem;"></i></a>
                    <div class="form-check form-switch d-inline-block">
                        <input class="form-check-input" type="checkbox" checked="checked" data-bs-toggle="tooltip" title="' . __d('two_geoloc', 'Voir ou masquer membres géoréférencés') . '" id="memberbox" /><label class="form-check-label" for="memberbox">' . $ic_b_mbgt . ' ' . __d('two_geoloc', 'Membre') . '</label>
                    </div>
                ...
                <div class="collapse" id="l_sb_member">
                    <div class="sb_member list-group-item py-1 border-left-0 border-right-0" >
                        <input id="n_filtrmb" placeholder="' . __d('two_geoloc', 'Filtrer les utilisateurs') . '" class="my-1 form-control form-control-sm" type="text" />
                    </div>
                    ' . $sidebarmembres . '
                </div>
                </div>';
...
$f = fopen("modules/" . $ModPath . "/storage/json/data.json", "w");
$w = fwrite($f, $file_json);
@fclose($f);
@chmod('modules/' . $ModPath . '/storage/json/data.json', 0666);
//<== construction du fichier json
$f = fopen("modules/" . $ModPath . "/storage/json/user.geojson", "w");
...
$fond_provider = array(
    ...
    ['modisterra', __d('two_geoloc', 'Satellite') . ' (NASA)'],
...
$ecr_scr .= '
    var src_countries = new ol.source.Vector({
        url: "/modules/geoloc/storage/json/countries.geojson",
...
$ecr_scr .= '
        ...
        button.setAttribute("title", "' . __d('two_geoloc', 'Masquer') . '")
        var sidebarSwitch = function(e) {
            if($("#sidebar").hasClass("show")) {
                $("#sidebar").collapse("toggle");
                button.innerHTML = "&#xf0d7";
                button.setAttribute("data-bs-original-title", "' . __d('two_geoloc', 'Voir') . '")
            }
            else {
                $("#sidebar").collapse("show");
                button.innerHTML = "&#xf0d8";
                button.setAttribute("data-bs-original-title", "' . __d('two_geoloc', 'Masquer') . '")
            }
        };
...
if ($username != '') {
    if ($ue_lat != '' and $ue_long != '') {
        $infooo = '<div id="oldloc"><strong>' . __d('two_geoloc', 'Coordonnées enregistrées :') . '</strong><br /><span class="text-muted">' . __d('two_geoloc', 'Latitude') . ' :</span> ' . $ue_lat . '<br /><span class="text-muted">' . __d('two_geoloc', 'Longitude') . ' :</span> ' . $ue_long . '<br /></div><br /><strong>' . __d('two_geoloc', 'Voulez vous changer pour :') . '</strong><br />';
        $mess_mb = __d('two_geoloc', 'Cliquer sur la carte pour modifier votre position.');
    } else {
        $infooo = '<div id="newloc"><strong>' . __d('two_geoloc', 'Vous n\'êtes pas géoréférencé.') . '</strong></div><br /><strong>' . __d('two_geoloc', 'Voulez vous le faire à cette position:') . '</strong><br />';
        $mess_mb = __d('two_geoloc', 'Cliquer sur la carte pour définir votre géolocalisation.');
    }
    $sb_georef .= '   <div class="list-group-item list-group-item-action py-1 border-left-0 border-right-0"><div class="form-check form-switch"><input class="form-check-input" type="checkbox" data-bs-toggle="tooltip" title="' . __d('two_geoloc', 'Définir ou modifier votre position.') . '" id="georefbox" /><label for="georefbox" class="form-check-label">' . __d('two_geoloc', 'Définir ou modifier votre position.') . '</label></div><span class="help-block small muted"><span style="display: inline-block; transform: rotate(45deg);"><i class="bi bi-pin me-1 h3 text-primary"></i></span>' . $mess_mb . '</span></div>';
    ...
    //==> Georeferencement par user
        ...
        map.on("click", function(evt) {
            ...
            $(element).popover({
                ...
                animation: false,
                html: true,
                title:\'<span class="fw-light">' . __d('two_geoloc', 'Géolocalisation') . '</span><button type="button" id="close" class="btn-close float-end" onclick="$(\\\'.popover\\\').hide();"></button>\',
            ...
            + \'' . $infooo . '\'
            + \'<div id="lalo"><span class="text-muted">' . __d('two_geoloc', 'Latitude') . ' : </span>\' + lat.toFixed(6) + \'<br /><span class="text-muted">' . __d('two_geoloc', 'Longitude') . ' : </span>\' + lng.toFixed(6) + \'</div>\'
            + \'<button type="submit" class ="btn btn-primary btn-sm mt-2">' . __d('two_geoloc', 'Enregistrez') . '</button>\'
    ...
    $(document).ready(function () {
        centeronMe = function(u) {
            ...
            if(u.substr(0,1) == "u") {
                ...
                $("#ol_popup").show();
                container.innerHTML = \'<img class="me-2 img-thumbnail n-ava" src="\' + mb.get("ava") + \'" align="middle" /><span class="lead">\' + mb.get("pseudo") + \'</span><div class="my-2">' . __d('two_geoloc', 'Dernière visite') . ' : \' + mb.get("lastvisit") + \'</div><hr /><div class="text-center lead">\' + mb.get("userlinks") + \'</div>\';
...
$ecr_scr .= '
                ...
                containertip.attr("data-bs-original-title", "' . __d('two_geoloc', 'Cliquer sur la carte pour modifier votre position.') . '").tooltip("show");
        ...
        var feature = map.forEachFeatureAtPixel(evt.pixel, function (feature, layer) {
            if (feature) {
                ...
                if(layerid =="utilisateurs")
                    container.innerHTML = \'<img class="me-2 img-thumbnail n-ava" src="\' + feature.get("ava") + \'" align="middle" /><span class="lead">\' + feature.get("pseudo") + \'</span><div class="my-2">' . __d('two_geoloc', 'Dernière visite') . ' : \' + feature.get("lastvisit") + \'</div><hr /><div class="text-center lead">\' + feature.get("userlinks") + \'</div>\';
                ...
                if(layerid =="anony") {
                    if(feature.get("ip_qui") == "A")
                        container.innerHTML = \'' . $ic_b_acg . ' \' + feature.getId() + \' @ \' + feature.get("ip_hostaddr") + \' <br /><hr /><span class="text-muted">' . __d('two_geoloc', 'Hôte') . ' : </span>\'+ feature.get("ip_hote") +\'<br /><span class="text-muted">' . __d('two_geoloc', 'En visite ici') . ' : </span> \' + feature.get("ip_visitpage") +\'<br /><span class="text-muted">' . __d('two_geoloc', 'Visites') . ' : </span>[\' + feature.get("ip_visit") +\']<br /><span class="text-muted">' . __d('two_geoloc', 'Ville') . ' : </span>\'+ feature.get("ip_city") +\'<br /><span class="text-muted">' . __d('two_geoloc', 'Pays') . ' : </span>\'+ feature.get("ip_country") +\'<hr /><img src="\'+ feature.get("ip_flagsrc") +\'.png" class="n-smil" alt="flag" />\' || "(unknown)";
                    else
                        container.innerHTML = \'' . $ic_b_mbgc . ' \' + feature.get("ip") + \' @ \' + feature.get("ip_hostaddr") + \' <br /><hr /><span class="text-muted">' . __d('two_geoloc', 'Hôte') . ' : </span>\'+ feature.get("ip_hote") +\'<br /><span class="text-muted">' . __d('two_geoloc', 'En visite ici') . ' : </span> \' + feature.get("ip_visitpage") +\'<br /><span class="text-muted">' . __d('two_geoloc', 'Visites') . ' : </span>[\' + feature.get("ip_visit") +\']<br /><span class="text-muted">' . __d('two_geoloc', 'Ville') . ' : </span>\'+ feature.get("ip_city") +\'<br /><span class="text-muted">' . __d('two_geoloc', 'Pays') . ' : </span>\'+ feature.get("ip_country") +\'<hr /><img src="\'+ feature.get("ip_flagsrc") +\'.png" class="n-smil" alt="flag" />\' || "(unknown)";
                }
                } else {
                ...
                if (cfeatures.length > 1) {
                    ...
                    for (var i = 0; i < cfeatures.length; i++) {
                        ...
                        switch(cfeatures[i].get("spam")) {
                            case "" : infospam = ""; break;
                            case "5" : infospam = \'<br /><span class="text-danger">' . __d('two_geoloc', 'Adresse IP bannie !') . ' [\'+ cfeatures[i].get("spam") +\']</span>\'; break;
                            default : infospam = \'<br /><span class="text-danger">' . __d('two_geoloc', 'Adresse IP signalée par l’antispam !') . ' [\'+ cfeatures[i].get("spam") +\']</span>\'; break;
                        }
                        l_ip += \'<div class=""><a class="\'+cl+\'" href="#div\' + cfeatures[i].getId() + \'" data-bs-toggle="collapse">\' + cfeatures[i].get("ip") + \'</a></div><div class="collapse small border-bottom pb-1 mb-1" id="div\' + cfeatures[i].getId() + \'"><span class="text-muted">' . __d('two_geoloc', 'Visites') . ' : </span>[\' + cfeatures[i].get("visit") + \']<br /><img class="n-smil me-2" alt="flag" src="\' + cfeatures[i].get("flag") + \'.png" loading="lazy"/>\' + cfeatures[i].get("pays") + \' \' + cfeatures[i].get("city") + \'<br /><span class="text-muted">' . __d('two_geoloc', 'Posts/Commentaires') . ' : </span>[\' + cfeatures[i].get("post") + \']\'+infospam+\'</div>\';
                    }
                    l_ip += \'</div>\';
                ...
                if (cfeatures.length == 1) {
                    ...
                    switch(cfeatures[0].get("spam")) {
                        case "" : infospam = ""; break;
                        case "5" : infospam = \'<br /><hr /><span class="text-danger">' . __d('two_geoloc', 'Adresse IP bannie !') . ' [\'+ cfeatures[0].get("spam") +\']</span>\'; break;
                        default : infospam = \'<br /><hr /><span class="text-danger">' . __d('two_geoloc', 'Adresse IP signalée par l’antispam !') . ' [\'+ cfeatures[0].get("spam") +\']</span>\'; break;
                    }
                    container.innerHTML = \'<div class="small"><span class="\' + cl + \'"><i class="me-1 bi bi-display h4 align-middle"></i>@ \' + cfeatures[0].get("ip") + \'</span><hr /><span class="text-muted">' . __d('two_geoloc', 'Visites') . ' : </span>[\' + cfeatures[0].get("visit") + \']<br /><img class="n-smil me-2" alt="flag" src="\' + cfeatures[0].get("flag") + \'.png" loading="lazy"/>\' + cfeatures[0].get("pays")+ \' \' + cfeatures[0].get("city")+ \'<br /><span class="text-muted">' . __d('two_geoloc', 'Posts/Commentaires') . ' : </span>[\' + cfeatures[0].get("post") + \']\'+infospam+\'</div>\';
                }
                }
...
$affi = '
    ...
    <h3 class="mt-4 mb-3">' . __d('two_geoloc', 'Géolocalisation des membres du site') . '<span class="float-end"><span class="badge bg-secondary me-2" title ="' . __d('two_geoloc', 'Membres du site') . '" data-bs-toggle="tooltip" data-bs-placement="left">' . $total_membre . '</span></span></h3>
    <div class=" mb-4">
        <div id="map-wrapper" class="ol-fullscreen my-3">
            ...
            <div id="sidebar" class= "collapse show col-sm-4 col-md-3 col-6 px-0">
                ...
                <div class="bg-light text-dark fw-light p-2"><a class="link" data-bs-toggle="collapse" href="#l_sb_tools"><i class="toggle-icon fa fa-caret-down fa-lg me-2" style="font-size:1.6rem;"></i></a>' . __d('two_geoloc', 'Fonctions') . '<span class="float-end">' . $lkadm . '</span></div>
                <div class="collapse" id="l_sb_tools">
                    ' . $sb_georef . '
                    <div class="list-group-item list-group-item-action py-1 border-left-0 border-right-0">
                        <div class="mb-3 row">
                            <label class="col-form-label col-sm-12" for="cartyp"><span class="align-middle bi bi-layers-half me-2 h3 text-primary"></span>' . __d('two_geoloc', 'Type de carte') . '</label>
                            <div class="col-sm-12">
                            ...
                            <label class="my-0 float-end small form-label" for="baselayeropacity">' . __d('two_geoloc', 'Opacité') . '</label>
                        ...
                        <hr class="mb-1"/>
                        <div><span class="align-middle bi bi-layers me-2 h3 text-primary"></span>' . __d('two_geoloc', 'Couches utilitaires') . '</div>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" data-bs-toggle="tooltip" title="' . __d('two_geoloc', 'Voir ou masquer la grille') . '" id="grillebox" />
                            <label class="form-check-label" for="grillebox">' . __d('two_geoloc', 'Grille') . '</label>
                        </div>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" data-bs-toggle="tooltip" title="' . __d('two_geoloc', 'Voir ou masquer la couche') . '" id="coastandborder" />
                            <label class="form-check-label" for="coastandborder">' . __d('two_geoloc', 'Côtes et frontières') . '</label>
                        </div>
...
$affi .= '
        ...
        <ul class="nav nav-tabs mt-4">
            <li class="nav-item"><a id="messinfo-tab" class="nav-link active" href="#infocart" data-bs-toggle="tab_ajax"><span class="d-sm-none"><i class=" fa fa-globe fa-lg me-2"></i><i class=" fa fa-info fa-lg"></i></span><span class="d-none d-sm-inline">' . __d('two_geoloc', 'Infos carte') . '</span></a></li>
            <li class="nav-item"><a id="aide-tab" class="nav-link" href="modules/geoloc/doc/aide_geo-' . $language . '.html" data-bs-target="#aide" data-bs-toggle="tab_ajax"><span class="d-sm-none"><i class=" fa fa-globe fa-lg me-2"></i><i class=" fa fa-question fa-lg"></i></span><span class="d-none d-sm-inline">' . __d('two_geoloc', 'Aide') . '</span></a></li>';
if (users::autorisation(-127) and $geo_ip == 1)
    $affi .= '
            <li class="nav-item"><a id="iplist-tab" class="nav-link " href="#ipgeolocalisation" data-bs-toggle="tab_ajax"><span class="d-sm-none"><i class=" fa fa-globe fa-lg me-2"></i><i class=" fa fa-tv fa-lg"></i></span><span class="d-none d-sm-inline">' . __d('two_geoloc', 'Ip liste') . '</span></a></li>';
$affi .= '
        </ul>
    <div class="tab-content">
        ...
        <div class="tab-pane fade mt-2" id="ipgeolocalisation">
            <h5 class="mt-3">
                <i title="' . __d('two_geoloc', 'IP géoréférencées') . '" data-bs-toggle="tooltip" style="color:' . $acg_t_co . '; opacity:' . $acg_t_op . ';" class="fa fa-desktop fa-lg me-2 align-middle"></i>
                <span class="badge bg-secondary me-2 float-end">' . $ipnb . '</span>
            </h5>
            <div class="mb-3 row">
                <div class="col-sm-12">
                <input type="text" class="mb-3 form-control form-control-sm n_filtrbox" placeholder="' . __d('two_geoloc', 'Filtrer les résultats') . '" />
                </div>
            </div>
        ...
        <div class="tab-pane fade" id="tracker">
            <input type="checkbox" title="' . __d('two_geoloc', 'Voir ou masquer les waypoints') . '" id="wpobox" />&nbsp;' . __d('two_geoloc', 'Voir ou masquer les waypoints') . ' <span id="envoyer">Ex</span>
            <input type="checkbox" title="' . __d('two_geoloc', 'Activer désactiver la géolocalisation') . '" id="geolobox" onclick="" />&nbsp;' . __d('two_geoloc', 'Activer désactiver la géolocalisation') . '
        </div>
    </div>
    </div>';
echo $affi . $ecr_scr;
