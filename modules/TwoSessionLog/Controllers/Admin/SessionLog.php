...
namespace Modules\TwoSessionLog\Controllers\Admin;
...
class SessionLog extends AdminController
{
    ...
    {
        $this->f_meta_nom = 'ession_log';

        $this->f_titre = __d('two_session_log', 'Gestion des Logs');
    ...
    public function index(Request $request)
    {
        ...
        $f_titre = __d('two_session_log', 'Gestion des Logs');
        ...
        echo '
            ...
            <li class="nav-item"><a href="' . $ThisFile . '&subop=session" class="nav-link ' . $cl_a_ses . '">' . __d('two_session_log', 'Liste des Sessions') . '</a></li>
            <li class="nav-item"><a href="' . $ThisFile . '&subop=security" class="nav-link ' . $cl_a_sec . '">' . __d('two_session_log', 'Liste des Logs') . '</a>
        ...
        if ($subop == 'session') {
            echo '
                <br />
                <h3>' . __d('two_session_log', 'Liste des Sessions') . ' : <code>TABLE session</code></h3>
                <table id="tad_ses" data-classes="table table-sm table-striped table-borderless" data-toggle="table" data-show-toggle="true" data-search="true" data-mobile-responsive="true" data-buttons-class="outline-secondary" data-icons-prefix="fa" data-icons="icons">
                    <thead>
                        <tr>
                        <th class="n-t-col-xs-1" data-halign="center" data-align="center" data-sortable="true">' . __d('two_session_log', 'Nom') . '</th>
                        <th class="n-t-col-xs-2" data-halign="center" data-align="right" data-sortable="true">@ IP</th>
                        <th class="n-t-col-xs-2" data-halign="center" data-align="right" data-sortable="true">' . __d('two_session_log', '@ IP résolue') . '</th>
                        <th data-halign="center" data-sortable="true">URI</th>
                        <th class="n-t-col-xs-1" data-halign="center" data-align="center" data-sortable="true">' . __d('two_session_log', 'Agent') . '</th>
                        </tr>
                    </thead>
                    <tbody>';
        ...
        if ($subop == 'info') {
            echo '
                <br />
                <h3>' . __d('two_session_log', 'Informations sur l\'IP') . '</h3>';
            ...
            echo '
                <div class="card card-body">
                    <div class="row">
                        <div class="col mb-3">
                        <span class="text-muted">' . __d('two_session_log', '@ IP') . '</span> : <span>' . $theip . '</span><br />
                        <span class="text-muted">' . __d('two_session_log', '@ IP résolue') . '</span> : <span>' . $hostname . '</span><br />
                        <span class="text-muted">' . __d('two_session_log', 'Fournisseur') . '</span> : <span>' . $provider . '</span><br />
                        </div>';
        ...
        if ($subop == 'mailog') {
            ...
            $subject = html_entity_decode(__d('two_session_log', 'Fichier de Log de'), ENT_COMPAT | ENT_HTML401, 'utf-8') . ' ' . $sitename;
            $message = __d('two_session_log', 'Fichier de Log de') . ' ' . $sitename . "<br /><br />";
        ...
        if ($subop == 'security') {
                ...
                if (filesize($FileSecure) != 0) {
                    ...
                    while (!feof($fd)) {
                        ...
                        <tr>
                            ...
                            <td><a href="' . $ThisFile . '&amp;subop=info&amp;theip=' . $ip . '" >' . __d('two_session_log', 'Infos') . '</a></td>
                        </tr>';
                            }
                        }
                    }
            ...
            echo '
                <h3 class="my-3"><a data-bs-toggle="collapse" href="#tog_tad_slog" aria-expanded="false" aria-controls="tog_tad_slog"><i class="toggle-icon fa fa-caret-down"></i></a><span class="ms-2">' . __d('two_session_log', 'Liste des Logs') . ' ' . __d('two_session_log', 'SECURITE') . ' : <code>security.log</code></span></h3>
                ...
                <h3 class="mt-3"><a data-bs-toggle="collapse" href="#tog_tad_tlog" aria-expanded="false" aria-controls="tog_tad_tlog"><i class="toggle-icon fa fa-caret-down"></i></a><span class="ms-2">' . __d('two_session_log', 'Liste des Logs') . ' ' . __d('two_session_log', 'TELECHARGEMENT') . ' : <code>security.log</code></span></h3>
        ...
        if ($subop == "upload") {
            ...
            if ($FileUpload != $FileSecure) {
                ...
                echo __d('two_session_log', 'Liste des Logs') . ' ' . __d('two_session_log', 'TELECHARGEMENT') . ' : <code class="code">' . $FileUpload . '</code>';
                echo '
                    <table>
                    ' . $UpLog . '
                    </table>';
    ...
    function action_log($ThisFile, $logtype)
    {
        ...
        $task = '
            <a class="dropdown-item" href="' . $ThisFile . '&amp;subop=mailog&amp;log=' . $whatlog . '"><i class="fa fa-at me-1 fa-lg"></i>' . __d('two_session_log', 'Recevoir le fichier par mail') . '</a>
            <a class="dropdown-item" href="' . $ThisFile . '&amp;subop=vidlog&amp;log=' . $whatlog . '"><i class="fa fa-times me-1 fa-lg"></i>' . __d('two_session_log', 'Vider le fichier') . '<br /><small>' . $FileSecure . '</small></a>
            <div class="dropdown-divider"></div>
            <a class="dropdown-item" href="' . $ThisFile . '&amp;subop=vidtemp"><i class="fas fa-trash me-1 fa-lg"></i>' . __d('two_session_log', 'Effacer les fichiers temporaires') . '<br /><small>' . $rep_cache . '</small></a>';
        return $task;
    }
