...
function marquetapage()
{
    ...
    if ($cookie[0] != '') {
        ...
        if (sql_num_rows($result)) {
            $content .= '
    <h6>
    <a class="mb-2" data-bs-toggle="collapse" data-bs-target="#lst_fav" id="show_fav" title="' . __d('two_marque_page', 'Déplier la liste') . '"><i id="i_lst_fav" class="fa fa-caret-down fa-lg toggle-icon text-primary me-2" ></i></a><span class="align-top">Bookmarks</span><span class="badge bg-secondary float-end">' . sql_num_rows($result) . '</span>
    </h6>
    <div id="lst_fav" class="collapse" >
        <a class="float-end" href="modules.php?ModPath=marquetapage&amp;ModStart=marquetapage&amp;op=supp_all&amp;uri=' . $_SERVER['PHP_SELF'] . '"><i class="fas fa-trash text-danger fa-lg" title="' . __d('two_marque_page', 'Effacer') . '" data-bs-toggle="tooltip" data-bs-placement="left"></i></a><br /><hr />';
            while (list($uri, $topic) = sql_fetch_row($result)) {
                $content .= '
        <div class="row g-0 my-2">
            <div class="col-11 n-ellipses"><a href="' . $uri . '" class="small ">' . $topic . '</a></div>
            <div class="col-1"><a class="float-end" href="modules.php?ModPath=marquetapage&amp;ModStart=marquetapage&amp;op=supp&amp;uri=' . urlencode($uri) . '"><i class="fas fa-trash text-danger" title="' . __d('two_marque_page', 'Effacer') . '" data-bs-toggle="tooltip"></i></a></div>
        </div>';
            }
            $content .= '
    </div>';
        ...
        $boxTitle = '<span class="me-2 fs-4"><a href="modules.php?ModPath=marquetapage&amp;ModStart=marquetapage&amp;op=add&amp;uri=' . $uri . '&amp;topic=' . urlencode($title_MTP) . '"><i class="fas fa-bookmark align-middle" title="' . __d('two_marque_page', 'Ajouter') . ' ' . __d('two_marque_page', 'favori') . '" data-bs-toggle="tooltip"></i></a></span>';
