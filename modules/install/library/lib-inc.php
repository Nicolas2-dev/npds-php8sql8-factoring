...
if (version_compare(PHP_VERSION, '5.3.0') >= 0 and extension_loaded('mysqli')) {
    ...
    include_once('library/database/mysql.php');
...
function verif_chmod()
{
    ...
    $file_to_check = array('abla.log.php', 'config/cache.config.php', 'config/config.php', 'config/filemanager.php', 'storage/logs/security.log', 'storage/meta/meta.php', 'storage/static/edito.txt', 'modules/upload/config/upload.conf.php');
    $i = 0;
    $listfich = '';
    ...
    if ($adminlogin != '') {
        if ($adminpass1 != $adminpass2)
            ...
            if (strlen($adminpass1) < $minpass)
                ...
                copy("modules/f-manager/config/modele.admin.conf.php", "modules/f-manager/config/" . strtolower($adminlogin) . ".conf.php");
                if (!$result1)
                    $stage7_ok = 0;
            }
        }
