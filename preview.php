   ...
   if(array_key_exists('user_avatar', $theposterdata)) {
      if ($theposterdata['user_avatar'] != '') {
         if (stristr($theposterdata['user_avatar'],"users_private"))
            ...
            if ($ibid=theme_image("forum/avatar/".$theposterdata['user_avatar'])) {$imgtmp=$ibid;} else {$imgtmp="assets/images/forum/avatar/".$theposterdata['user_avatar'];}
         }
          echo '
      ...
      echo '
                      <a style="position:absolute; top:1rem;" tabindex="0" data-bs-toggle="popover" data-bs-html="true" data-bs-title="'.$anonymous.'" data-bs-content=\''.$anonymous.'\'><img class=" btn-secondary img-thumbnail img-fluid n-ava" src="assets/images/forum/avatar/blank.gif" alt="icone '.$anonymous.'" /></a>';
   }
}
...
if (isset($image_subject)) {
   if ($ibid=theme_image("forum/subject/$image_subject")) {$imgtmp=$ibid;} else {$imgtmp="assets/images/forum/subject/$image_subject";}
   echo '<img class="n-smil" src="'.$imgtmp.'" alt="icone du post" />';
   } else {
      if ($ibid=theme_image("forum/icons/posticon.gif")) {$imgtmpP=$ibid;} else {$imgtmpP="assets/images/forum/icons/posticon.gif";}
