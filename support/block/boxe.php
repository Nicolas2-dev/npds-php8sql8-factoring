...
class boxe
{
    ...
    public static function online(): void
    {
        ...
        if ($user) {
            ...
            $result = cache::Q_select3(
                DB::table('users')
                    ->select('uid')
                    ->where('uname', $username)
                    ->get(), 86400, crypt::encrypt('(users_uid_uname)'));
            
            $numrow = DB::table('priv_msgs')
                        ->select('to_userid')
                        ->where('to_userid', $result[0]['uid'])
    ...
    public static function adminblock(): void
    {
        ...
        if ($admin) {
            ...
            $content = language::aff_langue(preg_replace_callback('#<a href=[^>]*(&)[^>]*>#', [str::class, 'changetoampadm'], $res_block['content']));
            ...
            if ($versus_info[1] == Config::get('versioning.Version_Sub') and $versus_info[2] == Config::get('versioning.Version_Num')) {
                DB::table('fonctions')->where('fid', 36)->update(array(
                    'fetat'         => 1,
                    'fretour'       => '',
                    'fretour_h'     => 'Version NPDS '. Config::get('versioning.Version_Sub') .' '. Config::get('versioning.Version_Num'),
                    'furlscript'    => '',
                ));
            } else {
                DB::table('fonctions')->where('fid', 36)->update(array(
    ...
    public static function userblock(): void
    {
        if ((users::getUser()) and (users::cookieUser(8))) {
            ...
            global $block_title;
            $title = $block_title == '' ? translate("Menu de") . ' ' . users::cookieUser(1) : $block_title;
                ...
                if ($numrows > 0) {
                    ...
                    $boxstuff .= (($cat == $storie['catid'])
                        ...
                        : '<li class="list-group-item list-group-item-action hyphenate">
                                <a href="'. site_url('index.php?op=newcategory&amp;catid='. $storie['catid']) .'" data-bs-html="true" data-bs-toggle="tooltip" data-bs-placement="right" title="'. translate("Dernière contribution") .' <br />'. date::formatTimestamp($res['time']) .' ">
    ...
    public static function bloc_langue(): void
    {
        ...
        if (Config::get('npds.multi_langue')) {
            ...
            themesidebox($title, language::aff_local_langue(site_url('index.php'), "choice_user_language", ''));
        }
    }
